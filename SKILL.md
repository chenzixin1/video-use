...
└── edit/
    ...
    ├── transcripts/<name>.json  ← cached provider JSON normalized to `words`
...
- A transcription provider resolves. Default is ElevenLabs via `ELEVENLABS_API_KEY`. For Volcengine/ByteDance, set `TRANSCRIBE_PROVIDER=volcengine` plus `MODEL_SPEECH_API_KEY` or `VOLCENGINE_API_KEY`; optional `MODEL_SPEECH_APP_ID`/`VOLCENGINE_APP_ID` selects the old app-key auth path. Volcengine uses HTTP base64 local-file upload, not object storage.
...
- **`transcribe.py <video>`** — single-file transcription. Default `--provider elevenlabs`; use `--provider volcengine` for ByteDance/Volcengine BigModel ASR standard HTTP. `--num-speakers N` applies to ElevenLabs. Cached unless `--force`.
- **`transcribe_batch.py <videos_dir>`** — 4-worker parallel transcription. Supports `--provider volcengine`. Use for multi-take.
...
- **Running Whisper locally on CPU.** Slow and it normalizes fillers. Use a hosted word-level ASR provider.
